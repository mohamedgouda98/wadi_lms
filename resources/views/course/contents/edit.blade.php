...
@section('content')
<form id="content_edit_form" action="{{ route('classes.contents.update', $content->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-9">
            <select class="form-control @error('content_type') is-invalid @enderror" required id="val-Content"
                ...
                <option value="Video" id="youtube" {{ $content->content_type == 'Video' ? 'selected' : '' }}>
                    @translate(Video)
                </option>
                <option value="Document" id="vimeo" {{ $content->content_type == 'Document' ? 'selected' : '' }}>
                    @translate(Document)
                </option>
                @if(quizActive())
            ...
            <div class="form-group row">
                ...
                <div class="col-lg-9">
                    <select class="form-control lang @error('provider') is-invalid @enderror" id="val-provider"
                        ...
                        <option value="Youtube" {{ $content->provider == 'Youtube' ? 'selected' : '' }}>
                            @translate(Youtube)
                        </option>
                        <option value="HTML5" {{ $content->provider == 'HTML5' ? 'selected' : '' }}>
                            @translate(HTML5)
                        </option>
                        <option value="Vimeo" {{ $content->provider == 'Vimeo' ? 'selected' : '' }}>
                            @translate(Vimeo)
                        </option>
                        <option value="File" {{ $content->provider == 'File' ? 'selected' : '' }} id="file">
                            @translate(File)
                        </option>
                        <option value="Live" {{ $content->provider == 'Live' ? 'selected' : '' }}>
                            @translate(Live)
                        </option>
                    </select>
                ...
                <div class="col-lg-9">
                    ...
                    @if (zoomActive())
                        ...
                        <select class="form-control lang @error('meeting_id') is-invalid @enderror" id="val-meeting"
                            ...
                            @foreach (App\Models\Meeting::where('user_id', Auth::user()->id)->latest()->get() as $course)
                                <option value="{{ $course->id }}">
                                    {{ $course->meeting_title }}
                                </option>
                            @endforeach
                            {{-- @foreach (App\Models\Course::Published()->where('user_id', Auth::user()->id)->with('meeting')->get() as $course)
