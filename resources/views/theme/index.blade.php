            ...
            <div class="col-md-12">
                <div class="card m-2">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary ml-3" href="{{ route('theme.manager.installui') }}"
                    ...
                    <div class="card-header">
                        @if (env('THEME_MANAGER') == 'YES')
                            <h4><strong>@translate(Installed Theme)(s) ( {{ App\Models\Themes::count() }} )</strong></h4>
                        @else
                                    ...
                                    <div class="news">
                                        <figure class="article">
                                            <a href="javscript:void()"
                                                onclick="forModal('{{ route('addon.preview', $theme->name) }}', '{{ strtoupper($theme->name) }}')">
                                                <img src="{{ filePath($theme->image) }}" class="w-100 img-fluid rounded"
                                                    alt="#{{ $theme->name }}">
                                            </a>
                                        </figure>
                                        <a href="{{ route('theme.status', $theme->name) }}"
                                            class="btn {{ $theme->activated == 0 ? 'btn-success' : 'btn-danger disable' }} w-100">{{ $theme->activated == 0 ? 'Activate' : 'Deactivate' }}</a>
                                    </div>
                                </div>
                            @empty
                                <div class="col-12">
        ...
        <script>
            $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    url: 'https://api.envato.com/v1/market/new-files-from-user:' + username + ',' + site +
                        '.json',
                        ...
                        data.forEach(element => {
                            if (element.tags.match('courselms theme')) {
                                addons += '<div class="col-4">' +
                                    '<div class="news">' +
                                    '<figure class="article">' +
                                    '<img src=' + element.live_preview_url +
                                    ' class="w-100 rounded" alt="#' + element.item + '">' +
                                    ' </figure>' +
                                    '<a href=' + element.url +
                                    ' class="btn btn-primary w-100" "target=_blank">Buy Now</a>' +
                                    '</div>' +
                                    '</div>'
                                addons_count += 1;
                            }
