...
class Choices
{
    ...
    public function createQuestionAnswer($args)
    {
        ...
        if(! $this->questionHasAnswers($examQuestionId))
        {
        ...
        $answer = QuestionAnswer::create([
            ...
            'answer' => $data['answer'],
            'correct' => $this->questionHasAnswers($examQuestionId) ? 0 : 1
        ]);
    ...
    {
        $exam = $args['exam'];
        return ExamQuestion::with('answers')->where('exam_id', $exam->id)->where('active', 1)->inRandomOrder()->limit($exam->limit_questions)->get();
    }
    ...
    public function teacherMark($args)
    {
        ...
        $studentExam = StudentExam::find($args['data']['student_exam_id'])->update([
            'degree' => $totalDegree,
            'is_marked' => 1
        ]);
