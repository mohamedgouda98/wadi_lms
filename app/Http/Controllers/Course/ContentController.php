...
class ContentController extends Controller
{
    ...
    */
    ...
    public function edit($id)
    {
        ...
        $classes = Classes::where('course_id', $content->course_id)->get();
        $quizes = collect();
        if (quizActive()) {
            $q = Quiz::where('status', 1)->where('user_id', Auth::id())->where('course_id', $content->course_id)->get();
            foreach ($q as $i) {
                if ($i->questions->count() > 0) {
                    $quizes->push($i);
                }
        ...
        return view('course.contents.edit', compact( 'content', 'classes', 'quizes'));
    }
    /*
    |--------------------------------------------------------------------------
