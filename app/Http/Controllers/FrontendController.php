...
use App\Models\CoursePurchaseHistory;
use App\Models\Demo;
...
class FrontendController extends Controller
{
    ...
    {
        if (config('courselms.active_theme') != 'frontend') {
            if ($request->key == null) {
                $courses = null;
            } else {
            ...
            if ($search == null) {
                ...
                $courses = Course::Published()->where('title', 'LIKE', "%{$request->search}%")->get();

                return view($this->theme.'.course.search', compact('courses', 'search'));
            }
        }
    }
    ...
    public function affiliateCreate()
    {
        ...
        $history = null;
        $payment = null;
        $affiliate = \App\Models\Affiliate::where('user_id', Auth::id())->first();
        if ($affiliate) {
    ...
    {
        $affiliate = \App\Models\Affiliate::where('user_id', Auth::id())->firstOrFail();
    ...
    public function affiliatePaymentStore(Request $request)
    {
        ...
        $ins = \App\Models\Affiliate::where('user_id', Auth::id())->first();
